...
export async function fetchNewColor() {
  ...
  const closeHexCode = data.name.closest_named_hex;
  const colorName = data.name.value;

  setColorToGuess(closeHexCode, colorName);

  console.log(
    "What does the color »Royal Blue« look like? You matched the color Royal Blue with an accuracy of 97.33%"
  );
