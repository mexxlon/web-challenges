...
document.addEventListener("DOMContentLoaded", () => {
  ...
  inputColor.addEventListener("input", () => {
    colorBox.style.backgroundColor = `hsl(${inputColor.value}, 70%, 50%)`;
  });

  inputRadius.addEventListener("input", () => {
    colorBox.style.borderRadius = `${inputRadius.value}%`;
  });

  inputRotation.addEventListener("input", () => {
    colorBox.style.transform = `rotate(${inputRotation.value}deg)`;
  });
});
